...
TEST_CASE("cache flusher persists cache") {
  ...
  public:
    HttpResponse get_with_headers(const std::string &, const std::vector<std::string> &) override {
      return {"[]", {"ETag: abc123"}, 200};
    }
    std::string get(const std::string &, const std::vector<std::string> &) override { return ""; }
